...
(function (global, setTimeout) {
     ...
     */
    function each(ary, func) { //工具函数  遍历数组
        if (ary) {
            var i;
     ...
     */
    function eachReverse(ary, func) { //工具函数  反向遍历数组
        if (ary) {
            var i;
     ...
     */
    function eachProp(obj, func) {  //遍历对象属性，去除原型链上的属性
        var prop;
        for (prop in obj) {
            if (hasProp(obj, prop)) {
    ...
    function getGlobal(value) { //获取全局对象上的属性
        if (!value) {
            return value;
        }
        var g = global;
    ...
    if (typeof define !== 'undefined') { //如果define已经定义，结束函数
        //If a define is already in play via another AMD loader,
        //do not overwrite.
        return;
    }

    if (typeof requirejs !== 'undefined') { //如果requirejs已经定义
        if (isFunction(requirejs)) { //如果requirejs是函数，解决冲突，使用cfg缓存requirejs变量
    ...
    if (typeof require !== 'undefined' && !isFunction(require)) { //如果require已经定义，解决冲突，使用cfg缓存requirejs变量
    ...
    function newContext(contextName) {  //创建新的上下文环境
         ...
         * defQueue.
         */
        function takeGlobalQueue() { //将全局的def队列添加到当前上下文的def中队列
